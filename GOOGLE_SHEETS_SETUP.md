...
| `GOOGLE_SHEETS_CREDS` | ключ из шага 4 — **сырой JSON целиком** ИЛИ его **base64** |

**Важно:** значение `GOOGLE_SHEETS_CREDS` должно быть **всем файлом** — от
открывающей `{` до закрывающей `}`. Частая ошибка: вставить кусок без фигурных
скобок → `JSONDecodeError: Extra data`.

**Рекомендуется base64** (нет кавычек/переносов — на Railway ничего не ломается):

```bash
# macOS:
base64 -i ключ.json | pbcopy
# Linux:
base64 -w0 ключ.json
```
Вставь результат в `GOOGLE_SHEETS_CREDS`. Бэкенд принимает и сырой JSON, и base64.
